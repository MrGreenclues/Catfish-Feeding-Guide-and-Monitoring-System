...
<div class="main-content">
  ...
  <div class="form-container">
    <form method="POST" action="Efeedingconn.php" class="form-border">
    <div class="mb-3">
        <label for="item" class="form-label">Item</label>
        <input type="text" id="inputitem" name="item" class="form-control" placeholder="Item">
      </div>
      <div class="mb-3">
        <label for="feeding" class="form-label">Feeds intake</label>
        <input type="text" id="inputfeeding" name="feeding" class="form-control" placeholder="Feeds Intake">
      </div>
      <div class="mb-3">
        <label for="pond" class="form-label">Pond</label>
        <input type="text" id="inputpond" name="pond" class="form-control" placeholder="Pond">
      </div>
      <div class="mb-3">
      ...
      <button type="submit" name="update" value="<?=$feeds['id'];?>" class="btn btn-primary">Update</button>
  ...
  <table class="table">
    <thead>
            <tr>
            <input type="hidden" value="<?= $activity['act_id'] ?>" class="activity-id">
            <th scope="col">Item</th>
                <th scope="col">Feeds intake</th>
        ...
        <tbody id="tableBody">
            ...
            if(mysqli_num_rows($query_run)>0){
              while($feeds=mysqli_fetch_assoc($query_run)){
              ?>
                <tr> 
                <input type="hidden" value="<?= $feeds['id'] ?>" class="id">
                <td><?=$feeds['item'];?></td>
                  <td><?=$feeds['feeding'];?></td>
                  <td><?=$feeds['pond'];?></td>
                  <td><?=$feeds['time'];?></td>
...
.table-container {
  width: 100%;
  max-width: 1200px;
  min-height: 400px; /* Ensures a big box appearance */
  overflow: auto;
