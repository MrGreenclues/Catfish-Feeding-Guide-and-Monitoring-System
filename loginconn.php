...
$max_attempts = 2; // Maximum number of failed attempts before lockout
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($lockoutResult->num_rows >= $max_attempts) {
        ...
        echo '<script>alert("Too many failed login attempts. Please try again later."); window.location.href = "login.php";</script>';
        echo "<a href='login.php'><button class = 'back_btn ' type='button'>re-login</button></a>";
    ...
    if ($result_admin->num_rows == 1) {
        ...
        header("location: manageuser.php"); // Redirect to admin dashboard page after successful login
    } else {
